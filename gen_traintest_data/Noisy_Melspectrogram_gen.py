...
def get_melspectrogram(base_path, file_name):
    ...
    ### resample audio to 16kHz
        ...
        if sound.size() == noise.size():
            ...
            np.save(path[:-4] + "_SNR" + str(snr_db) + "_whitenoise" + "melspec" + ".npy", melspec)
            print("saved: %s"%path )
        else:
            continue

csv_path = r"C:\Users\user\Desktop\Intern_Joshua\kws_training2\folder"
get_melspectrogram(csv_path, "//train_clean.csv")
get_melspectrogram(csv_path, "//test_clean.csv")