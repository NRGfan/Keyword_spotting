...
class GoogleSpeechCommands():
    ...
    def __getitem__(self, index):
        ...
        mfcc = np.load(path[:-4] + "0noise" + "_25mfcc_denoised" + ".npy")
...
class kws_lstm(nn.Module):
    def __init__(self):
        ...
        self.lstm1 = nn.LSTM(input_size=25, hidden_size=86, batch_first=True, num_layers=2)
        self.fc1 = nn.Linear(86, NUM_CLASSES)
        self.relu = nn.ReLU()
    def forward(self, x):
        x, _ = self.lstm1(x)
        x = x[:, -1, :]
...
optimizer = optim.Adam(model.parameters(), lr = 0.000770257, weight_decay = 0.0001) ## weight decay before 0.0001
